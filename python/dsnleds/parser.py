...
class DSNParser(object):
    ...
    def get_url(self):
        return "https://eyes.jpl.nasa.gov/dsn/data/dsn.xml?r=%s" % (int)(
            time.time() / 5
        )
    ...
    def parse_signal(self, signal):
        ...
        if signal.get("power") == "":
            ...
            data["power"] = to_decimal(
                signal.get("power")
            )  # Power (in dBm for downlink, kW for uplink.)

        if signal.get("frequency") == "" or signal.get("frequency") == "none":
            data["frequency"] = None
        else:
            data["frequency"] = to_decimal(
                signal.get("frequency")
            )  # Frequency (Hz). Always present but may be wrong if type is none

        if signal.get("dataRate") == "":
            data["data_rate"] = None
        else:
            data["data_rate"] = to_decimal(
                signal.get("dataRate")
    ...
    def fetch_config(self):
        ...
        response = self.http_session.get(url)
        doc = etree.fromstring(response.content)
        spacecraft = self.fetch_spacecraft(
            doc.xpath("/config/spacecraftMap/spacecraft")
        )
